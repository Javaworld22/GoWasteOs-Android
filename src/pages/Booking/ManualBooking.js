...
export default class ManualBooking extends Component {
    constructor(props){
        super(props); 
        this.googleaddressRef = React.createRef();
        this.state = {
            citylist:[],
            servicelist:[],
            categorylist:[],
            service_provider_id:this.props.navigation.state.params.providerId,
            city:'',
            service:'',
            category:'',
            size:'',
            ...
            selectdate: new Date(),
            timestring:moment(new Date()).format('hh:mm A'),
            selecttime:moment(new Date()).format('HH:mm'),
            showDate: false,
            showTime: false,
            isCityError: false,
    ...
    _jobPost = async () => {
        ...
        formData.append("service_provided_city_id",this.state.city);
        formData.append("service_id",this.state.service);
        formData.append("price_id",this.state.category.split("/")[0]);
        formData.append("service_charge",this.state.category.split("/")[1]);
        ...
        this.setState({ showLoader: true });
        let response = await POST(endPoints.manualBooking, formData, {
          Authorization: await Retrieve("userToken")
        });
        this.setState({ showLoader: false });
        ...
        if (response.details.ack == 1) {
            ...
            this.googleaddressRef.current.setAddressText("");

            Alert.alert("", response.details.message, [
                { text: "Ok", onPress: () => this.props.navigation.navigate('BookingConfirmation',{confirmationId:response.details.confirmationId})}
            ]);
        } else {
                            ...
                            <View style={style.mT2}>
                                ...
                                <View style={style.roundinput}>
                                    <Picker
                                        ...
                                        onValueChange={(itemValue, itemIndex) => this._getServiceList(itemValue)}
                                    >
                                ...
                                <View style={style.roundinput}>
                                    <Picker
                                        ...
                                        onValueChange={(itemValue, itemIndex) => this._getCategoryList(itemValue)}
                                    >
                                ...
                                <View style={style.roundinput}>
                                    ...
                                    >
                                        ...
                                        {this.state.categorylist !== "" ? 
                                            ...
                                            return <Picker.Item label={categorylist.category+' (₦ '+categorylist.price+'/'+categorylist.size+' Liters)'} value={categorylist.id+'/'+categorylist.price} key={key} />;
                                            })
                                        :
                                            <Picker.Item label="Loading..." value="0" />
                                        }
                                ...
                                <View style={style.roundinput}>
                                    <TextInput  
                                        ...
                                        placeholderTextColor="#acacac" 
                                        editable={false}
                                        style={[style.forminput],{width:'50%', height:40}}
