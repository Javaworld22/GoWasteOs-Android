...
export default class JobEdit extends Component {
        ...
        if (response.details.ack == '1') {
            this.setState({jobDetails: response.details.job_details},
                () => {
                    ...
                    this.setState({city:response.details.job_details.city.id});
                    this.setState({service:response.details.job_details.service.id});
                    this.setState({category:response.details.job_details.price.id});
                    this.setState({size:response.details.job_details.garbage_size});
                    this.setState({address:response.details.job_details.location});
                    this.setState({lattitude:response.details.job_details.lattitude});
                    this.setState({longitude:response.details.job_details.longitude});
                    this.setState({datestring:moment(response.details.job_details.date).format('YYYY-MM-DD')});
                    this.setState({timestring:moment.utc(response.details.job_details.time).format('hh:mm A')});
                    this.setState({selecttime:moment.utc(response.details.job_details.time).format('HH:mm')});
                    this.myRef.current.setAddressText(response.details.job_details.location);
                }
            );
        }

        this._getCityList();
        this._getServiceList(response.details.job_details.city.id, 1);
        this._getCategoryList(response.details.job_details.service.id, 1);
    };
    ...
    _jobPost = async () => {
        ...
        formData.append("garbage_size",this.state.size);
        ...
        formData.append("date",this.state.datestring);
        formData.append("time",this.state.selecttime);
    
        this.setState({ showLoader: true });
        let response = await POST(endPoints.jobEdit, formData, {
          Authorization: await Retrieve("userToken")
        });
        this.setState({ showLoader: false });
        ...
        if (response.ack == 1) {
            Alert.alert("", response.message, [
                { text: "Ok", onPress: () => this.props.navigation.navigate('JobDetails',{jobId: this.state.jobId})}
            ]);
        } else {
                            ...
                            <View style={style.mT2}>
                                ...
                                <View style={[style.mT4,style.pB5]}>
                                    <TouchableOpacity style={style.themebtn} onPress={() => this._jobPost()}>
                                        <Text style={style.btntext}>Edit Job</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        </View>
